...
window.onload = function init()
{
	...
	document.getElementById("change").onclick = function () {
		texture_mode+= 1.0;
		if(texture_mode >= 4) texture_mode = 0.0;
		...
		{
			texture_tog = 1.0;
			image = document.getElementById("check");
			configureTexture( image );
		}		
	...
	textureLoc = gl.getUniformLocation(program,"texture_tog");
...
{	
	...
	for (var z = 100.0; z > -100.0; z -= 5.0) 
	{
		for (var x = -100.0; x < 100.0; x += 5.0) 
		{
			...
			objPoints.push(vec4(x, -1.0, z));
			planeCount++;
			objPoints.push(vec4(x-5.0, -1.0, z));
			planeCount++;
			objPoints.push(vec4(x, -1.0, z-5.0));
			planeCount++;
			objPoints.push(vec4(x, -1.0, z-5.0));
			planeCount++;
			objPoints.push(vec4(x-5.0, -1.0, z-5.0));
			planeCount++;
			objPoints.push(vec4(x-5.0, -1.0, z));
			planeCount++;
        ++i;
...
{	   
	...
	gl.uniform1f(textureLoc, texture_tog);
    ...
    gl.drawArrays( gl.TRIANGLES, planeCount, objPoints.length-planeCount );
